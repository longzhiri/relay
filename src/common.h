...
{
    return (uint64_t) * buf << 56 | ((uint64_t) * (buf + 1) << 48) | ((uint64_t) * (buf + 2) << 40) | ((uint64_t) * (buf + 3) << 32) | ((uint64_t) * (buf + 4) << 24) | ((uint64_t) * (buf + 5) << 16) | ((uint64_t) * (buf + 6) << 8) | ((uint64_t) * (buf + 7));
}
...
static uint32_t encode_session_key(uint16_t identity1, uint16_t identity2)
{
    if (identity1 < identity2) {
        return (uint32_t)identity1 << 16 | (uint32_t)identity2;
    } else {
        return (uint32_t)identity2 << 16 | (uint32_t)identity1;
    }
}
